// Keep the Google AdSense snippet in its separate HTML file.
(function() {
	var tagUrl = new URL('../../tag_google_adsense_code_20290914.html', document.currentScript.src);

	fetch(tagUrl, { cache: 'no-store' })
		.then(function(response) {
			if (!response.ok)
				throw new Error('AdSense tag request failed: ' + response.status);
		...
		.catch(function(error) {
			console.warn('Google AdSense could not be loaded.', error);
		});
})();
